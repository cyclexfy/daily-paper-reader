...
title_zh: SHAPE：面向医学图像分割的具有合理性评估的结构感知分层无监督领域自适应
...
## 摘要
无监督领域自适应（UDA）对于在不同临床环境中部署医学分割模型至关重要。现有方法存在根本局限性，受困于语义无关的特征对齐（导致分布保真度差）以及忽视全局解剖约束的伪标签验证，从而无法防止生成全局不合理的结构。为解决这些问题，我们提出了 SHAPE（具有合理性评估的结构感知分层无监督领域自适应），这是一个将自适应重新聚焦于全局解剖合理性的框架。该框架基于 DINOv3 基础模型，其分层特征调制（HFM）模块首先生成具有高保真度和类别感知能力的特征。这将核心挑战转移到了如何稳健地验证伪标签上。为了增强传统的像素级验证，我们引入了超图合理性估计（HPE），利用超图来评估标准图无法捕捉的全局解剖合理性。此外，还辅以结构异常剪枝（SAP），通过跨视图稳定性清除残留的伪影。SHAPE 在心脏和腹部跨模态基准测试中显著优于现有方法，在心脏数据上实现了 90.08% (MRI->CT) 和 78.51% (CT->MRI) 的 SOTA 平均 Dice 分数，在腹部数据上实现了 87.48% (MRI->CT) 和 86.89% (CT->MRI) 的分数。代码可在 https://github.com/BioMedIA-repo/SHAPE 获取。
