...
tldr: 针对医学影像数据稀缺及3D解剖结构生成一致性难的问题，本文提出Sketch2CT框架。该框架结合2D草图与文本描述，通过多模态扩散模型生成结构感知的3D医学影像。首先利用胶囊注意力机制融合草图与文本特征生成3D分割掩码，再以此引导潜在扩散模型合成高质量CT体积。该方法实现了可控且低成本的医学数据增强，在公开数据集上表现优异。
selection_source: fresh_fetch
motivation: 旨在解决在多模态条件下生成具有解剖结构一致性的3D医学影像这一复杂难题。
method: 提出一种结合2D草图与文本引导的扩散框架，先生成3D分割掩码，再通过掩码引导潜在扩散模型合成CT体积。
result: 在公开CT数据集上的实验证明，该方法在生成多模态医学体积方面具有卓越的性能和高度的可控性。
conclusion: Sketch2CT提供了一种高效、低成本且遵循解剖结构的医学数据增强方案，具有重要的临床应用潜力。
---

## 摘要
扩散概率模型在生成高质量、逼真的医学图像方面展现出巨大潜力，为医学领域长期存在的数据稀缺挑战提供了一个极具前景的解决方案。然而，在多模态条件下生成具有解剖结构一致性的 3D 医学体数据仍然是一个复杂且尚未解决的问题。我们提出了 Sketch2CT，这是一个用于结构感知 3D 医学体数据生成的多模态扩散框架，由用户提供的 2D 草图和捕捉 3D 几何语义的文本描述共同引导。该框架最初在两种模态的约束下，从随机噪声中生成目标器官的 3D 分割掩码。为了有效地对齐和融合这些输入，我们提出了两个关键模块，分别利用局部文本线索细化草图特征，并整合全局草图-文本表示。这些模块基于胶囊注意力（capsule-attention）骨干网络构建，利用草图和文本的互补优势来生成解剖学上准确的器官形状。随后，合成的分割掩码引导潜扩散模型进行 3D CT 体数据合成，从而实现与用户定义的草图和描述一致的器官外观真实重建。在公开 CT 数据集上的广泛实验表明，Sketch2CT 在生成多模态医学体数据方面表现出优越的性能。其可控、低成本的生成流程实现了医学数据集的有原则且高效的增强。代码可在 https://github.com/adlsn/Sketch2CT 获取。
