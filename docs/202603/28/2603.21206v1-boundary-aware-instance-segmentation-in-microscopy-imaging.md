...
tldr: 针对显微成像中密集接触或重叠细胞分割的难题，本文提出了一种无需提示词的边界感知实例分割框架。该方法通过预测有符号距离函数（SDF）而非传统的二进制掩码，实现了平滑且几何一致的轮廓建模。结合学习到的 Sigmoid 映射和统一的修正豪斯多夫距离（MHD）损失函数，该框架在保持边界精确性的同时，显著提升了相邻实例的分离效果，在多个数据集上优于现有的 SAM 等基础模型。
selection_source: fresh_fetch
motivation: 现有的分割模型（如 SAM）在处理显微镜图像中密集且重叠的细胞实例时，往往难以在无提示的情况下实现准确分离。
method: 提出一种预测有符号距离函数（SDF）的框架，利用学习到的映射函数和 MHD 损失来优化边界定位和区域一致性。
result: 在公开和私有高通量显微数据集上的评估显示，该方法在边界精度和实例分割性能上均优于最新的基础模型。
conclusion: 该研究证明了基于 SDF 的边界感知建模在解决密集生物医学图像实例分割挑战中的有效性。
---
