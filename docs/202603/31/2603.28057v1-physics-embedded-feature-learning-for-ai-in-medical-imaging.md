...
tags: ["query:cpath"]
score: 8.0
evidence: 物理嵌入深度学习用于肿瘤生长与分类
tldr: 针对医疗影像中深度学习模型缺乏物理一致性和可解释性的问题，本文提出PhysNet框架。该框架将肿瘤生长的反应扩散物理模型嵌入ResNet的中间特征学习过程，在执行多分类任务的同时，学习肿瘤密度场及扩散率、生长率等生物物理参数。实验证明，PhysNet在脑部MRI数据集上的表现优于多种主流模型，为构建可信且具有临床意义的医疗AI提供了新路径。
selection_source: fresh_fetch
motivation: 现有的深度学习医疗影像模型多为黑盒，忽略了肿瘤生长的物理规律，导致其缺乏可解释性和临床信任。
method: 提出PhysNet框架，将肿瘤生长的反应扩散模型直接嵌入卷积神经网络的中间特征层，实现物理规律与数据驱动特征的深度融合。
result: 在大型脑部MRI数据集上的实验显示，PhysNet在分类准确率和F1分数上均优于MobileNetV2、VGG等主流模型及集成模型。
conclusion: 物理嵌入式特征学习不仅提升了模型性能，还通过学习生物物理参数增强了医疗AI的可解释性和物理一致性。
---

## 摘要
深度学习（DL）模型在智能医疗领域取得了强劲的表现，但大多数现有方法作为黑盒运行，忽略了控制肿瘤生长的物理过程，从而限制了其可解释性、鲁棒性和临床信任。为了解决这一局限性，我们提出了 PhysNet，这是一种物理嵌入式深度学习框架，它将肿瘤生长动力学直接集成到卷积神经网络（CNN）的特征学习过程中。与仅在输出层施加物理约束的传统物理信息方法不同，PhysNet 将肿瘤生长的反应扩散模型嵌入到 ResNet 主干网络的中间特征表示中。该架构在执行多类肿瘤分类的同时，通过端到端训练学习潜在肿瘤密度场、其时间演化以及具有生物学意义的物理参数（包括肿瘤扩散率和生长率）。这种设计是必要的，因为纯数据驱动模型即使在高度准确或基于集成学习的情况下，也无法保证物理一致的预测，也无法提供对肿瘤行为的见解。在大型脑部 MRI 数据集上的实验结果表明，PhysNet 优于多个最先进的深度学习基准模型（包括 MobileNetV2、VGG16、VGG19 和集成模型），实现了卓越的分类准确率和 F1 分数。除了性能提升外，PhysNet 还生成了可解释的潜在表示和与既有医学知识相一致的学习生物物理参数，突显了物理嵌入式表示学习是通向更可靠且具有临床意义的医疗人工智能系统的实用路径。
