...
## 摘要
多模态学习整合了来自图像、文本和音频等不同模态的互补信息以提升模型性能，但其成功依赖于大规模标注数据，而获取这些数据的成本十分昂贵。主动学习（AL）通过选择性地标注具有信息量的样本来缓解这一挑战。在多模态设置中，许多方法隐含地假设模态重要性在各轮次中是稳定的，并在融合阶段保持选择规则固定，这使得它们对多模态学习的动态特性不敏感，即模态的相对价值和实例的难度会随着训练的进行而发生变化。为了解决这一问题，我们提出了 RL-MBA，这是一个用于模态平衡且难度感知的多模态主动学习的强化学习框架。RL-MBA 将样本选择建模为一个马尔可夫决策过程，其策略能够适应模态贡献、不确定性和多样性，而奖励机制则鼓励准确率的提升和平衡。两个关键组件驱动了这种适应性：(1) 自适应模态贡献平衡（AMCB），通过强化反馈动态调整模态权重；(2) 用于难度感知策略调整的证据融合（EFDA），通过基于不确定性的证据融合来估计样本难度，从而优先选择具有信息量的样本。在 Food101、KineticsSound 和 VGGSound 上的实验表明，RL-MBA 始终优于强基准方法，在有限的标注预算下同时提高了分类准确率和模态公平性。
