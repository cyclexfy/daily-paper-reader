...
title_zh: FDIF：基于隐式函数的公式驱动监督学习用于 3D 医学图像分割
...
tags: ["query:cpath"]
score: 6.0
evidence: 无需真实数据标签的医学图像分割可扩展预训练
tldr: 针对3D医学图像分割中大规模标注数据获取难的问题，本文提出FDIF框架。该框架利用隐式函数（SDF）通过数学公式生成合成数据进行预训练，无需真实数据或专家标注。FDIF能更精确地建模复杂几何形状并合成逼真纹理，在多个基准测试中表现优异，甚至可媲美基于大规模真实数据的自监督学习方法，为无数据表征学习提供了新范式。
selection_source: fresh_fetch
motivation: 现有的公式驱动监督学习方法在几何表达能力和纹理合成方面存在局限，难以模拟复杂的医学图像特征。
method: 引入基于有符号距离函数（SDF）的隐式函数表示，实现对复杂几何形状的紧凑建模以及几何与强度纹理的可控合成。
result: 在多个医学影像分割基准和架构上，FDIF显著优于现有公式驱动方法，并达到了与大规模真实数据预训练相当的性能。
conclusion: 基于隐式函数的公式监督学习是一种极具潜力的无数据表征学习范式，能有效提升3D医学影像分割和分类任务的性能。
---

## 摘要
基于深度学习的 3D 医学图像分割方法依赖于大规模标注数据集，但由于隐私限制和专家标注的高昂成本，获取此类数据非常困难。公式驱动监督学习 (FDSL) 提供了一种极具吸引力的替代方案，它直接从数学公式生成训练数据和标签。然而，现有的基于体素的方法在几何表达能力上有限，且无法合成逼真的纹理。我们提出了基于隐式函数的公式驱动监督学习 (FDIF)，这是一个无需使用任何真实数据和医学专家标注即可实现可扩展预训练的框架。FDIF 引入了基于有符号距离函数 (SDF) 的隐式函数表示，能够对复杂几何形状进行紧凑建模，同时利用 SDF 的表面表示来支持几何和强度纹理的可控合成。在三个医学图像分割基准（AMOS、ACDC 和 KiTS）以及三种架构（SwinUNETR、nnUNet ResEnc-L 和 nnUNet Primus-M）上，FDIF 相比于公式驱动方法有一致的提升，并达到了与在大规模真实数据集上预训练的自监督方法相当的性能。我们进一步展示了 FDIF 预训练也有益于 3D 分类任务，突显了基于隐式函数的公式监督是无数据表示学习中一种极具前景的范式。代码可在 https://github.com/yamanoko/FDIF 获取。
