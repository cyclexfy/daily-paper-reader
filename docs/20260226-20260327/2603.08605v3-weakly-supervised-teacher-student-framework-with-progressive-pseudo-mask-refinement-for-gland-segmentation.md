...
tags: ["query:cpath"]
score: 8.0
evidence: 结直肠癌组织病理学中的弱监督分割
tldr: 针对结直肠癌组织病理学中腺体分割标注成本高的问题，本研究提出一种弱监督教师-学生框架。该框架利用稀疏标注和指数移动平均（EMA）稳定的教师网络，通过置信度过滤、自适应融合和课程引导细化，逐步生成高质量伪掩码。实验证明该方法在减少标注工作量的同时，在多个数据集上实现了优异的分割性能和良好的泛化能力。
