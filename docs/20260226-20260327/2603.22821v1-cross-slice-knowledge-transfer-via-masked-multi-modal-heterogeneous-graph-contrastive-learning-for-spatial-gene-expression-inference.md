...
tags: ["query:cpath"]
score: 9.0
evidence: 病理基础模型与多模态图用于空间基因表达
tldr: 空间转录组学（ST）成本高昂，从病理图像预测基因表达是有效替代方案，但现有方法难以捕捉跨切片的复杂空间关系。本文提出SpaHGC，一种基于多模态异构图的模型，通过病理基础模型提取图像嵌入，捕捉切片内和切片间的点对关系。结合掩码图对比学习，SpaHGC实现了从参考切片到目标切片的知识迁移，显著提升了预测精度和生物学相关性。
selection_source: fresh_fetch
motivation: 针对现有病理图像预测空间基因表达方法在捕捉跨切片复杂空间依赖性方面的不足。
method: 提出SpaHGC模型，利用多模态异构图和掩码图对比学习技术，整合切片内空间上下文与跨切片图像相似性。
result: 在七个不同平台和组织的基准数据集上，SpaHGC在所有评估指标上均显著优于九种现有的最先进方法。
conclusion: 该研究证明了跨切片知识迁移在提升基因表达预测准确性方面的有效性，为癌症研究提供了具有强生物学相关性的工具。
---

## 摘要
虽然空间转录组学（ST）增进了我们对组织背景下基因表达的理解，但其高昂的实验成本限制了其大规模应用。从病理图像预测 ST 是一种极具前景且具有成本效益的替代方案，但现有方法难以捕捉复杂的跨切片空间关系。为应对这一挑战，我们提出了 SpaHGC，这是一种基于多模态异构图的模型，能够从组织学图像中捕捉切片内和切片间的位点（spot）关系。它整合了目标切片内的局部空间上下文，以及由病理基础模型提取的图像嵌入计算出的跨切片相似性。这些嵌入实现了切片间的知识迁移，SpaHGC 进一步结合了掩码图对比学习（Masked Graph Contrastive Learning）以增强特征表示，并将空间基因表达知识从参考切片迁移到目标切片，使其能够建模复杂的空间依赖关系并显著提高预测准确性。我们在来自不同平台、组织和癌症亚型的七个匹配的组织学-ST 数据集上进行了全面的基准测试。结果表明，SpaHGC 在所有评估指标上均显著优于现有的九种最先进方法。此外，预测结果在多个癌症相关通路中显著富集，从而突显了其强大的生物学相关性和应用潜力。
