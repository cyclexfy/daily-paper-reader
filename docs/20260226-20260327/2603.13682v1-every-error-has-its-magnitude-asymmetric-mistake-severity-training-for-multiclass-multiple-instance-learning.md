...
tags: ["query:cpath"]
score: 10.0
evidence: 用于全切片图像诊断的多类多实例学习
tldr: 针对多分类多实例学习（MIL）在全切片图像诊断中忽视错误严重程度的问题，本文提出了一种错误严重性感知训练策略。该方法通过构建分层诊断结构并结合严重性加权交叉熵损失，重点惩罚高风险误诊。同时，引入概率对齐和语义特征重混以增强鲁棒性，并提出非对称评估指标。实验证明该方法能显著减少临床关键错误，且在医学和自然领域数据上均表现优异。
selection_source: fresh_fetch
motivation: 现有的多实例学习框架在多分类任务中未能区分不同误诊类型的严重程度，导致临床上的关键错误无法得到有效处理。
method: 提出一种基于分层结构的错误严重性感知训练策略，利用加权损失惩罚严重错误，并通过概率对齐和特征重混确保分层一致性。
result: 在公开和内部医学数据集以及自然领域数据上的实验表明，该方法显著降低了严重误诊率，优于现有方法。
conclusion: 该研究通过引入错误严重性维度，有效提升了多实例学习在临床诊断中的安全性和可靠性，并展现了跨领域的适用潜力。
---
