...
title_zh: 像病理学家一样行动：组织感知的全切片图像推理
...
tags: ["query:cpath"]
score: 9.0
evidence: 用于病理问答的视觉语言模型
tldr: "针对病理全切片图像（WSI）问答中信息冗余和计算效率低的问题，本文提出了一种模拟病理医生观察习惯的框架HistoSelect。该框架采用由粗到精的检索策略，通过问题引导的组织感知采样和补丁选择，仅提取最关键的视觉信息。实验表明，该方法在减少70%视觉Token的同时提升了问答准确率，为构建高效、可解释的病理视觉语言模型提供了新思路。"
selection_source: fresh_fetch
motivation: 现有的病理问答模型通常采用均匀采样或全局注意力，无法像人类医生那样高效地从海量像素中定位关键病变区域。
method: 提出HistoSelect框架，包含识别相关组织区域的组采样器和检索区域内最具信息量补丁的选择器，实现由粗到精的视觉推理。
result: "在35.6万个问答对上的评估显示，该方法在平均减少70%视觉Token使用的前提下，显著提升了三项病理问答任务的准确性。"
conclusion: 将类人的搜索和注意力模式引入WSI推理，是构建实用且可靠的病理视觉语言模型的一个极具前景的方向。
---

## 摘要
近年来，在领域特定图像编码器以及利用视觉语言模型回答疾病相关自然语言问题的日益增长的兴趣推动下，计算病理学取得了飞速发展。然而，考虑到一张十亿像素的切片包含的信息远超回答特定问题所需，病理问答背后的核心问题仍未解决。病理学家在处理复杂的组织和形态时，通常会先进行大范围扫描，然后根据临床问题有选择性地放大观察。相比之下，目前的模型依赖于均匀的补丁（patch）采样或宽泛的注意力图，往往对无关区域给予同等关注，却忽略了关键的视觉证据。在这项工作中，我们试图使模型更接近人类实际检查切片的方式。我们提出了一个问题引导、组织感知且由粗到精的检索框架 HistoSelect，它由两个关键组件组成：一个用于识别与问题相关的组织区域的分组采样器（group sampler），以及一个用于在这些区域内检索最具信息量补丁的补丁选择器（patch selector）。通过仅选择最具信息量的补丁，我们的方法显著提高了效率：平均减少了 70% 的视觉 token 使用量，同时提高了三项病理问答任务的准确性。在 356,000 个问答对上的评估结果表明，我们的方法优于现有方法，并能生成基于可解释且与病理学家观察一致的区域的答案。我们的结果表明，将类人的搜索和注意力模式引入全切片图像（WSI）推理，是构建实用且可靠的病理视觉语言模型（VLM）的一个极具前景的方向。
