...
title_zh: 重新审视小补丁约束下基础模型在细胞级组织病理图像分析中的作用——训练数据规模和模糊扰动对 CNN 和 Vision Transformer 的影响
...
tags: ["query:cpath"]
score: 9.0
evidence: 细胞级组织病理图像分析中的基础模型
tldr: 本研究探讨了在极小图像块（40x40像素）约束下，基础模型与任务特定架构在细胞级病理图像分析中的表现。通过对18.5万张结直肠癌细胞图像进行实验，对比了CNN、ViT及多种基础模型在不同数据规模下的性能与鲁棒性。结果表明，随着数据量增加，针对小图像块优化的任务特定模型（如CustomViT）在准确率和效率上均优于基础模型，挑战了大型预训练模型在极小空间分辨率任务中的普适性。
selection_source: fresh_fetch
motivation: 探究在极小图像分辨率（40x40像素）的细胞级病理分析中，现代深度学习架构和基础模型是否具备鲁棒且可扩展的表征能力。
method: 利用18.5万张标注细胞图像，对比评估了8种从头训练的任务特定架构与3种经线性探测或微调的基础模型在不同数据规模下的表现。
result: 任务特定模型随数据规模增加持续提升，其中CustomViT表现最优且推理成本更低，而基础模型在适中样本量下即出现性能饱和。
conclusion: 在极小空间约束的细胞分类任务中，当训练数据充足时，任务特定架构比大型预训练基础模型更有效且更具效率。
---

## 摘要
背景与目标：细胞级病理图像分析需要处理极小的图像补丁（40x40 像素），远低于标准的 ImageNet 分辨率。目前尚不清楚现代深度学习架构和基础模型在此约束下是否能学习到鲁棒且可扩展的表示。我们系统地评估了小补丁细胞分类的架构适用性和数据规模效应。方法：我们分析了 303 份具有 CD103/CD8 免疫组化染色的结直肠癌标本，生成了 185,432 张标注的细胞图像。在多个数据规模（FlagLimit：每类 256--16,384 个样本）下从头训练了八种特定任务架构，并在将输入调整为 224x224 像素后，通过线性探测和微调评估了三个基础模型。使用调整大小前后的高斯扰动评估了对模糊的鲁棒性。结果：特定任务模型随着数据规模的增加而持续改进，而基础模型在中等样本量时趋于饱和。一种针对小补丁优化的 Vision Transformer (CustomViT) 达到了最高准确率，在推理成本大幅降低的情况下优于所有基础模型。各架构的模糊鲁棒性相当，未观察到基础模型具有定性优势。结论：对于极端空间约束下的细胞级分类，一旦有足够的训练数据，特定任务架构比基础模型更有效且更高效。较高的干净准确率并不意味着更优的鲁棒性，大型预训练模型在小补丁领域提供的收益有限。
