...
tags: ["query:cpath"]
score: 8.0
evidence: 用于结直肠癌分级的弱监督腺体分割
tldr: 针对结直肠癌组织病理学中腺体分割依赖大量像素级标注的问题，本文提出一种弱监督教师-学生框架。该框架利用稀疏标注，通过指数移动平均（EMA）稳定的教师网络、置信度过滤及课程引导细化，逐步生成高质量伪掩码，有效解决了类激活图（CAM）掩码不完整的问题，在多个数据集上展现了高效的标注利用率和良好的泛化能力。
selection_source: fresh_fetch
motivation: 传统的深度学习腺体分割方法依赖耗时的像素级标注，而现有的弱监督方法生成的伪掩码往往不完整且仅关注判别性区域。
method: 提出一种结合稀疏标注的教师-学生框架，通过指数移动平均稳定教师网络，并利用置信度过滤和课程引导机制逐步优化伪掩码。
result: 在GlaS数据集上达到80.10的mIoU和89.10的Dice系数，并在TCGA等跨队列评估中表现出较强的鲁棒性。
conclusion: 该框架为结直肠组织病理学腺体分割提供了一种标注高效且泛化性强的解决方案。
---

## 摘要
背景与目标：结直肠癌的组织病理学分级依赖于腺体结构的准确分割。目前的深度学习方法依赖于大规模像素级标注，这在常规临床实践中既费力又难以获得。弱监督语义分割提供了一个极具前景的替代方案。然而，基于类激活图的方法通常会产生不完整的伪掩码，这些掩码仅强调高度判别性的区域，而无法对未标注的腺体结构提供监督。我们提出了一种弱监督教师-学生框架，该框架利用稀疏的病理学家标注和指数移动平均（EMA）稳定的教师网络来生成细化的伪掩码。方法：该框架集成了基于置信度的过滤、教师预测与有限真值的自适应融合，以及课程引导的细化，以逐步分割未标注的腺体区域。该方法在来自俄亥俄州立大学韦克斯纳医学中心的机构结直肠癌队列（包含 60 张苏木精-伊红染色的全视野切片图像）以及包括 Gland Segmentation 数据集、TCGA COAD、TCGA READ 和 SPIDER 在内的公共数据集上进行了评估。结果：在 Gland Segmentation 数据集上，该框架实现了 80.10 的平均交并比（mIoU）和 89.10 的平均 Dice 系数。跨队列评估表明，在没有额外标注的情况下，该框架在 TCGA COAD 和 TCGA READ 上表现出鲁棒的泛化能力，而在 SPIDER 上性能的下降则反映了领域偏移。结论：所提出的框架为结直肠组织病理学中的腺体分割提供了一种标注高效且具有泛化能力的方法。
