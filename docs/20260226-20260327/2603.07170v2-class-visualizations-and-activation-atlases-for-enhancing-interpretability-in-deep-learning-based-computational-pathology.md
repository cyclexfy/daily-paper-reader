...
tags: ["query:cpath"]
score: 10.0
evidence: 评估了计算病理学中基于 Transformer 的基础模型的可解释性
...
## 摘要
基于 Transformer 的模型在计算病理学中的快速应用，使得从 H&E 全切片图像中预测分子和临床生物标志物成为可能，但其可解释性的发展尚未跟上模型复杂度的提升。虽然基于归因和生成的方法较为常见，但诸如类别可视化 (CVs) 和激活图谱 (AAs) 等特征可视化方法尚未针对这些模型进行系统评估。我们开发了一个可视化框架，并在具有递增标签粒度的组织和多器官癌症分类任务中，评估了基于 Transformer 的基础模型的 CVs 和 AAs。四名病理学家对真实和生成的图像进行了标注，以量化观察者间的一致性，并辅以归因和相似性指标。CVs 保留了形态差异显著组织的识别度，但在重叠的癌症亚类中表现出可分离性下降。在组织分类中，一致性从 Fleiss k = 0.75（扫描图）下降到 k = 0.31（CVs），在癌症亚类任务中也呈现类似趋势。AAs 揭示了依赖于层的组织结构：粗粒度的组织级概念形成了连贯区域，而更细分的亚类则表现出分散和重叠。组织分类的一致性中等 (k = 0.58)，粗粒度癌症分组的一致性较高 (k = 0.82)，而亚类水平的一致性较低 (k = 0.11)。图谱的可分离性与专家对真实图像的一致性密切相关，这表明表征的模糊性反映了内在的病理复杂性。在低复杂度设置下，基于归因的指标接近专家的变异性，而感知和分布指标显示出有限的一致性。总之，概念级特征可视化揭示了基于 Transformer 的病理模型中结构化的形态流形，并为跨标签粒度的学习表征提供了以专家为中心的审查框架。
