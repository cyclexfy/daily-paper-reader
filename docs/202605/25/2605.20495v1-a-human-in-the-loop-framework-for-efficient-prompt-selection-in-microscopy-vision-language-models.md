...
## 速览
**TLDR**：针对显微镜图像分类中标注成本高的问题，本文提出一种人在回路的提示选择框架，将提示集构建视为目标驱动的主动学习问题，优先选择需验证的未标注图像。实验表明，在低资源条件下，该方法仅需平均20张标注图像即可达到100%测试准确率，显著减少专家验证工作量。 \
**Motivation**：降低显微镜图像分类中专家标注的高昂成本，同时保持模型性能。 \
**Method**：将提示集构建形式化为目标驱动的主动学习问题，结合三种互补选择标准，优先标注最有价值的未标记图像。 \
**Result**：在严格低资源限制下，平均仅需20张专家验证图像即可达到100%测试准确率，远少于随机选择。 \
**Conclusion**：该人在回路框架展示了生成式AI在生物医学图像分析中以人为本的应用，专家参与验证和优化模型输出，同时大幅降低标注成本。
