...
tags: ["query:cpath"]
score: 7.0
evidence: 面向生物医学任务的视觉基础模型参数高效微调
tldr: "参数高效微调在生物医学视觉任务中常采用单一低秩更新，但受限于窄子空间，难以兼顾全局共享结构与局部残差方向，导致外部域泛化不足。为此提出LoRSA框架，联合学习密集低秩全局组件与动态结构稀疏残差组件，前者捕获协调适配，后者提供训练中演化的补充修正。在乳腺密度分类中，LoRSA在外部数据集MammosighTR和RSNA上取得最佳macro-F1，较最强基线分别提升2.15和3.09个百分点。权重分析表明两组件约92%能量落在对方双侧奇异子空间外，证实学习方向高度互补，为提升生物医学模型跨域泛化提供了有效途径。"
source: arxiv
...
motivation: 单一低秩更新将所有任务变化限制在窄参数子空间，阻碍模型同时捕获全局共享结构与局部残差，导致医学图像外部域泛化不佳。
method: 提出LoRSA全局-残差适配框架，联合学习密集低秩全局组件与动态结构稀疏残差组件，并分析其表示能力与奇异子空间互补性。
result: 在乳腺密度分类中，源域VinDr-Mammo训练，外部域MammosighTR和RSNA上macro-F1分别达最佳，较最强基线提升2.15和3.09个百分点。
conclusion: 将适配能力划分为全局与残差两条路径可显著改善参数高效微调模型的跨域泛化，为生物医学视觉基础模型适配提供新思路。
---

## 摘要
参数高效微调使得视觉基础模型能够在有限计算资源下适应生物医学任务，但单一的低秩更新可能将所有任务特定变化限制在一个狭窄的参数子空间内。这种限制可能阻碍模型同时表示全局共享的任务结构和局部残差方向，而后者对于泛化到未见过的成像领域至关重要。我们提出了LoRSA，一种全局-残差适应框架，它联合学习一个稠密低秩组件和一个动态结构化稀疏低秩组件。稠密组件捕获全局协调的任务适应，而结构化组件提供互补的残差校正，其支持集在训练过程中演化。我们刻画了这种分解的表示能力、逼近性质、秩结构和奇异子空间互补性。我们使用DINOv3-Base评估了LoRSA在四类乳腺密度分类上的表现，以VinDr-Mammo作为源域，MammosighTR和RSNA作为未见过的外部域。LoRSA在内部验证集上保持竞争力，并在两个目标数据集上取得了最佳外部宏F1分数，在MammosighTR上比最强竞争方法提高了2.15个百分点，在RSNA上提高了3.09个百分点。权重矩阵分析进一步表明，每个适应组件约92%的能量位于另一个组件的双边奇异子空间之外，这表明两个组件学习了很大程度上互补的更新方向。这些结果表明，将适应能力组织为不同的全局路径和残差路径可以提高参数高效适应的生物医学视觉模型的外部域泛化能力。
