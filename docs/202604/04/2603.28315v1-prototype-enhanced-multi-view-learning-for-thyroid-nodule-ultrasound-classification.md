...
## 摘要
使用超声成像进行甲状腺结节分类对于早期诊断和临床决策至关重要；然而，尽管现有的深度学习方法在分布内数据上表现良好，但在跨不同超声设备或临床环境部署时，往往表现出有限的鲁棒性和泛化能力。这一局限性主要归因于甲状腺超声图像显著的异质性，这可能导致模型捕捉到伪相关性而非可靠的诊断线索。为应对这一挑战，我们提出了 PEMV-thyroid，这是一种原型增强的多视图学习框架。该框架通过从多个特征视角学习互补表示，并利用带有混合原型信息的基于原型的校正机制来优化决策边界，从而考虑了数据的异质性。通过将多视图表示与原型级引导相结合，所提方法能够在异质成像条件下实现更稳定的表示学习。在多个甲状腺超声数据集上的广泛实验表明，PEMV-thyroid 一贯优于现有最先进的方法，特别是在跨设备和跨域评估场景中，从而在现实临床环境中提高了诊断准确性和泛化性能。源代码可在 https://github.com/chenyangmeii/Prototype-Enhanced-Multi-View-Learning 获取。
