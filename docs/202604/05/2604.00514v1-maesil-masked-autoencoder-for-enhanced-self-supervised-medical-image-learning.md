...
score: 6.0
evidence: 自监督医学图像学习
tldr: 针对3D医学影像标注数据稀缺及现有自监督学习方法忽视空间连续性的问题，本文提出MAESIL框架。该方法引入“superpatch”3D块作为输入单元，并结合双重掩码策略的3D掩码自编码器，有效捕获轴向相干性和3D结构信息。实验证明，MAESIL在多个大型CT数据集上的重建性能显著优于传统自编码器模型，为3D医学影像预训练提供了高效方案。
source: arxiv
...
motivation: 现有的医学影像自监督学习方法常将3D扫描视为独立的2D切片，导致关键的轴向相干性和3D结构上下文信息丢失。
method: 提出一种基于“superpatch”3D块输入和双重掩码策略的3D掩码自编码器框架，旨在高效提取空间表征。
result: 在三大公开CT数据集上，MAESIL在PSNR和SSIM等关键重建指标上均显著优于AE、VAE和VQ-VAE等主流模型。
conclusion: MAESIL通过创新的3D结构化学习，为3D医学影像任务提供了一个稳健且实用的自监督预训练解决方案。
---

## 摘要
针对三维（3D）医学影像（如计算机断层扫描，CT）训练深度学习模型，从根本上面临着标注数据稀缺的挑战。虽然在自然图像上进行预训练很常见，但这会导致显著的领域偏移（domain shift），从而限制了性能。在无标注医学数据上进行自监督学习（SSL）已成为一种强大的解决方案，但主流框架往往未能充分利用 CT 扫描固有的 3D 特性。这些方法通常将 3D 扫描视为一组独立的 2D 切片集合，这种方法从根本上丢弃了关键的轴向连贯性和 3D 结构上下文。为了解决这一局限性，我们提出了用于增强型自监督医学图像学习的自编码器（MAESIL），这是一种旨在高效捕获 3D 结构信息的新型自监督学习框架。其核心创新是“superpatch”，这是一种基于 3D 块（chunk）的输入单元，平衡了 3D 上下文保留与计算效率。我们的框架将体数据划分为 superpatch，并采用带有双重掩码策略的 3D 掩码自编码器策略，以学习全面的空间表示。我们在三个不同的、大规模公开 CT 数据集上验证了我们的方法。实验结果表明，MAESIL 在 PSNR 和 SSIM 等关键重建指标上，相较于 AE、VAE 和 VQ-VAE 等现有方法表现出显著改进。这确立了 MAESIL 作为 3D 医学影像任务中一种稳健且实用的预训练解决方案。
