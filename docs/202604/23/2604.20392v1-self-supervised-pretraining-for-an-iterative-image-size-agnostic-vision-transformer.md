...
title_zh: 迭代式图像尺寸无关视觉 Transformer 的自监督预训练
authors: "Nedyalko Prisadnikov, Danda Pani Paudel, Yuqian Fu, Luc Van Gool"
date: 2026-04-22
pdf: "https://arxiv.org/pdf/2604.20392v1"
tags: ["query:cpath"]
score: 7.0
evidence: 用于视觉基础骨干网的自监督预训练
tldr: 本研究针对视觉Transformer在处理大尺寸图像时计算效率低且难以扩展的问题，提出了一种基于自监督学习的图像尺寸无关预训练框架。该方法结合了受视网膜中央凹启发的迭代式架构与DINO自蒸馏目标，通过高效的积分图像补丁提取技术，实现了在固定计算预算下处理任意分辨率图像的能力，并在ImageNet-1K等任务上取得了竞争性表现。
source: arxiv
...
motivation: 传统ViT在自监督预训练中受限于低分辨率，且计算复杂度随图像尺寸增加而剧增，难以高效处理高分辨率图像。
method: 提出一种基于DINO自蒸馏的“序列到全局”SSL框架，利用迭代处理多尺度缩放补丁的架构和积分图像提取技术实现分辨率无关性。
result: 在ImageNet-1K及下游分类任务中取得了竞争性性能，且无论输入分辨率如何，计算开销均保持恒定。
conclusion: 该研究证明了迭代式架构结合自监督学习能有效打破图像尺寸限制，为构建高效、通用的视觉基础模型提供了新方案。
---

## 摘要
视觉 Transformer (ViT) 在自监督学习 (SSL) 领域占据主导地位。尽管它们已被证明在大规模预训练中非常有效，但其计算效率较低，且随图像尺寸的扩展性较差。因此，像 DINO 这样的基础模型被限制在低分辨率处理上。最近，一种受中央凹启发的 Transformer 通过迭代处理固定大小的多缩放补丁上下文，实现了分辨率无关性。该模型通过监督学习展示了良好的前景，它利用了一种类似于循环的序列化过程，且无需通过时间反向传播。为了释放其作为基础骨干网络的潜力，我们引入了一种基于 DINO 自蒸馏目标的新型“序列到全局”自监督学习框架。在高效的积分图补丁提取方法的支持下，我们的方法实现了图像尺寸无关视觉编码器的大规模预训练。我们在 ImageNet-1K 和下游分类任务上取得了具有竞争力的性能，且无论输入分辨率如何，都能保持恒定的计算预算。
